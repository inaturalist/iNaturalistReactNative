...
describe( "UniversalSearch screen", ( ) => {
  ...
  describe( "default location options (empty location field)", ( ) => {
    ...
    it( "fills the field and stages the nearby intent when Nearby is tapped", async ( ) => {
      ...
      await actor.press( screen.getByTestId( "UniversalSearch.searchButton" ) );
      // Intent only: no coordinates. The results screen resolves them, so
      // permission/GPS state no longer influences what's staged here.
    ...
    it(
      ...
      async ( ) => {
        ...
        // Selecting nearby must not touch location resolution on this screen.
