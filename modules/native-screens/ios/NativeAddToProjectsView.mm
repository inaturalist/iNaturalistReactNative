#import "NativeAddToProjectsView.h"
...
@implementation NativeAddToProjectsView {
    UIView * _view;
...
{
    return concreteComponentDescriptorProvider<NativeAddToProjectsViewComponentDescriptor>();
}

- (instancetype)initWithFrame:(CGRect)frame
{
  if (self = [super initWithFrame:frame]) {
    static const auto defaultProps = std::make_shared<const NativeAddToProjectsViewProps>();
    _props = defaultProps;
...
{
    const auto &oldViewProps = *std::static_pointer_cast<NativeAddToProjectsViewProps const>(_props);
    const auto &newViewProps = *std::static_pointer_cast<NativeAddToProjectsViewProps const>(props);
