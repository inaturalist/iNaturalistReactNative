...
9. **`npx patch-package <pkg>` fails with no output** (`{status: 1, output: [null,null,null], ...}` thrown from `makePatch.js`): npm's `allow-remote` security setting (default disabled since npm ~10/12) blocks patch-package's temp-folder install when the lockfile resolves a dependency to a direct tarball URL rather than a plain registry semver range — common with scoped/monorepo-style packages like `@react-navigation/*`. Fix: create a temporary root `.npmrc` containing `allow-remote=all` (patch-package copies `.npmrc` into its temp install dir), regenerate the patch, then **delete the `.npmrc`** immediately after — don't leave it committed, it loosens a real supply-chain protection.
